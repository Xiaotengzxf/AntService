//
//  CommercialHandleViewController.swift
//  AntService
//
//  Created by 张晓飞 on 2017/7/18.
...
class CommercialHandleViewController: UIViewController , IQDropDownTextFieldDelegate {
    ...
    @IBAction func save(_ sender: Any) {
        ...
        var params = ["id"  : "\(wfId)" , "opinion" : opinion ?? "" , (bComerical ? "dispose_step" : "stepid") : hand_result , "filepath" : filepath]
        if !bFinished {
            params[bComerical ? "dispose_r_to" : "r_to"] = to_user
        }
        
        NetworkManager.installshared.upload(url: bComerical ? NetworkManager.installshared.appOppoSaveDispose : NetworkManager.installshared.appSavelaunch , params: params, data: (image != nil ? UIImageJPEGRepresentation(image!, 0.2) : nil)) {[weak self] (json, error) in
            hud.hide(animated: true)
            if let object = json {
                if let result = object["result"].int , result == 1000 {
                    Toast(text: "商机处理成功").show()
    ...
    func loadUser() {
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.appOppoDesig, params: ["id" : wfId , "hand_result" : hand_result]){
